        ...
        if(err!=null){
            ...
            for(var i=0; i<data.length;i++){
                ...
                list.push(<div className="item"><NavLink to = {"/trailer/"+data[i].id}><img onMouseOver={function(e){
                   ...
                   var b = e.target;
                   b.style.opacity =1;
                }} alt="trailer" src={"https://image.tmdb.org/t/p/w500"+image}></img>
                <p>{data[i].title}</p></NavLink>
                </div>)
                }else{
                list.push(<div className="item"><NavLink to = {"/trailer/"+data[i].id}><img alt="noImage" ></img><p>{data[i].title}</p></NavLink></div>)
